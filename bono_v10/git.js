...
exports.upload = function(project_name, user_name, user_email, handler) {
	...
	var task7 = function(callback)
	{
		...
		var cmd2 = "for i in `find . \( -name '*.java' -o -name '*.xml' -o -name '*.properties' -o -name '*.css' -o -name '*.js' -o -name '*.sql' -o -name '*.txt' \) -print` ; do ../user_data/build/e2u.sh $i ; done"
		var cmd3 = "mv * -rf ../user_data/projects/" + project_name + "/_" + user_name + "/";
...
var task8 = function(callback)
	...
	var task8 = function(callback){//rm zip, folder
...
exports.pull = function(project_name, user_name, pull_handler, socket) {
	...
	var task0 = function(callback)
	{
		...
		var cmd = "cd " + DIR_PROJECT_USER + " ; " + 'git fetch --all; git merge -m "merged after fetch"';
		var child = exec(cmd, function(error, stdout, stderr) {
