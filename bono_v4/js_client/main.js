...
/*
					$.get('/make_dir?path=' + $("#left_tree_hoverdItem").val(), function(data, status){
				...
				"sep1": "---------",
				"quit": {name: "Quit", icon: "quit", callback: function(){
	...
	$("#makeDirFile_btn").click(function(){
		...
		var path = $("#left_tree_hoverdItem").val() + name;
...
/*
	$('button').click(function() {
		$.get('/makeTree', function(data, status) {
			//console.log(data);
			$("#git_tree_container").empty();
			$("#git_tree_container").append(data);
		});
